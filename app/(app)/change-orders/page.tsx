...
export default async function ChangeOrdersListPage({ searchParams }: { searchParams: { q?: string; status?: string; sort?: string; dir?: string; m?: string; t?: string } }) {
  const q = searchParams?.q ?? ''
  const status = (searchParams?.status as 'draft'|'pending'|'approved'|undefined) ?? undefined
  const sort = (searchParams?.sort as 'created_at'|'co_number'|'status'|undefined) ?? 'created_at'
      ...
      <div className="rounded-md border">
        <table className="w-full text-sm">
          ...
          <tbody>
                  ...
                  {co.id ? (
                    (() => { const { ConfirmDialog } = require('@/components/ui/confirm-dialog'); return (
                      <ConfirmDialog
                        ...
                        confirmLabel="Delete"
                        action={async () => { 'use server'; const { deleteChangeOrderAction } = await import('@/app/(app)/change-orders/actions'); await deleteChangeOrderAction(co.id as string, '/change-orders?m=Change%20order%20deleted&t=success') }}
            ...
            {cos.length === 0 && (
              <tr>
                <td className="px-3 py-4 text-muted-foreground" colSpan={4}>No change orders</td>
              </tr>
            )}
          </tbody>
        </table>
